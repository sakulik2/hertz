...
- Theming is deliberately coupled: `themes.xml` sets `windowBackground` to `@color/hertz_window_background` and light status-bar icons to avoid a launch flash, and that colour is kept equal to `BackgroundDeep` in `ui/theme/Color.kt`, so `HertzTheme` must stay dark. Changing one side without the other reintroduces a colour-shift frame at launch or invisible status-bar icons. There is no `values-night`; the app is dark-only. Tuner semantics Material has no slot for (`inTune`/`slightlyOff`/`off`/`dialTrack`) live in `MaterialTheme.tunerColors`, not in `colorScheme`. `safeDrawingPadding()` handles the edge-to-edge enforcement that comes with targetSdk 35+.
